"""Run one fresh agent per Lite reclassification shard."""
...
def output_paths(directory: Path, shard: int) -> tuple[Path, Path]:
    return (
        directory / f"classification-shard-{shard:03d}.jsonl",
        directory / f"classification-shard-{shard:03d}-report.md",
    )


def validation_command(directory: Path, shard: int) -> list[str]:
    return [
        "uv", "run", "python", "scripts/validate_episode_reclassification_3135.py",
...
def prompt(directory: Path, shard: int) -> str:
    ...
    return f"""Reclassify every Episode Lite record in shard {shard:03d} under the adjudicated Atlas.

Read only:
- {directory_path}/RECLASSIFICATION_PROTOCOL.md
- {directory_path}/source-shard-{shard:03d}.md

Do not read prior memberships, other shards, conference outcomes, Deep outputs,
or previous classifications. Write only:
- {directory_path}/classification-shard-{shard:03d}.jsonl
- {directory_path}/classification-shard-{shard:03d}-report.md

Classify by operative inference endpoint. Do not force coverage and do not erase
valid multi-label logic. Preserve uncertain and unmapped cases. Run
`uv run python scripts/validate_episode_reclassification_3135.py --directory
{directory_path} --shard {shard}` and repair both outputs until validation
...
def run_shard(
    ...
    *,
    ...
    log_path = log_dir / f"classification-shard-{shard:03d}.log"
    command = [
    ...
    return Result(
        shard, reasoning_effort, status, started_at, now(), round(time.monotonic() - started, 3),
