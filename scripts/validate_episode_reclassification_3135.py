"""Validate one or all Episode Lite reclassification shards."""

from __future__ import annotations

import argparse
import json
import re
from collections import defaultdict
from pathlib import Path
from typing import Any


DEFAULT_DIR = Path("data/analysis/iclr/episode-reclassification-3135")
FITS = {"core", "variant", "boundary"}
CONFIDENCES = {"low", "medium", "high"}
CHAIN_FIELDS = {
    "inspected_objects", "observations", "reasoning_bridge", "judgments",
    "requested_tests_or_changes", "signatures", "missingness",
}


def _text(value: Any, minimum: int) -> bool:
    return isinstance(value, str) and len(value.strip()) >= minimum


def _load_jsonl(path: Path) -> list[dict[str, Any]]:
    rows = []
    for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        if not line.strip():
            continue
        try:
            row = json.loads(line)
        except json.JSONDecodeError as exc:
            raise ValueError(f"{path}:{line_number}: invalid JSON: {exc}") from exc
        if not isinstance(row, dict):
            raise ValueError(f"{path}:{line_number}: row must be an object")
        rows.append(row)
    return rows


def _source_episodes(path: Path) -> dict[str, dict[str, Any]]:
    source = path.read_text(encoding="utf-8")
    try:
        block = (
            source.split("## Episode Lite records", 1)[1]
            .split("```jsonl\n", 1)[1]
            .split("\n```", 1)[0]
        )
    except IndexError as exc:
        raise ValueError(f"missing Episode Lite JSONL block: {path}") from exc
    rows = [json.loads(line) for line in block.splitlines() if line.strip()]
    return {row["episode_id"]: row for row in rows}


def validate_shard(directory: Path, shard: int) -> dict[str, Any]:
    manifest = json.loads((directory / "manifest.json").read_text(encoding="utf-8"))
    manifest_row = next((row for row in manifest["shards"] if row["shard"] == shard), None)
    if manifest_row is None:
        return {"shard": shard, "error_count": 1, "errors": ["shard absent from manifest"]}
    expected = manifest_row["episode_ids"]
    source_by_id = _source_episodes(directory / f"source-shard-{shard:03d}.md")
    pattern_ids = set(manifest["pattern_ids"])
    output_path = directory / f"classification-shard-{shard:03d}.jsonl"
    report_path = directory / f"classification-shard-{shard:03d}-report.md"
    errors: list[str] = []
    if not output_path.exists():
        return {"shard": shard, "error_count": 1, "errors": [f"missing {output_path}"]}
    if not report_path.exists() or len(report_path.read_text(encoding="utf-8").strip()) < 500:
        errors.append(f"missing or too-short report: {report_path}")
    try:
        rows = _load_jsonl(output_path)
    except (ValueError, OSError) as exc:
        return {"shard": shard, "error_count": 1, "errors": [str(exc)]}
    ids = [row.get("episode_id") for row in rows]
    if ids != expected:
        errors.append("output episode IDs must match manifest exactly and in source order")

    reason_owners: dict[str, set[str]] = defaultdict(set)
    member_count = uncertain_count = unmapped_count = audit_count = 0
    for index, row in enumerate(rows, 1):
        episode_id = row.get("episode_id", f"record-{index}")
        prefix = f"{episode_id}:"
        required = {
            "episode_id", "memberships", "uncertain_patterns",
            "closest_excluded_patterns", "unmapped_logic",
            "classification_confidence", "needs_source_audit",
        }
        if set(row) != required:
            errors.append(f"{prefix} top-level keys differ")
        memberships = row.get("memberships", [])
        uncertain = row.get("uncertain_patterns", [])
        closest = row.get("closest_excluded_patterns", [])
        if not all(isinstance(value, list) for value in (memberships, uncertain, closest)):
            errors.append(f"{prefix} membership/uncertainty/exclusion values must be lists")
            continue
        if len(closest) > 3:
            errors.append(f"{prefix} closest_excluded_patterns exceeds three")
        member_ids = [item.get("pattern_id") for item in memberships if isinstance(item, dict)]
        uncertain_ids = [item.get("pattern_id") for item in uncertain if isinstance(item, dict)]
        closest_ids = [item.get("pattern_id") for item in closest if isinstance(item, dict)]
        for label, values in (
            ("memberships", member_ids), ("uncertain_patterns", uncertain_ids),
            ("closest_excluded_patterns", closest_ids),
        ):
            if len(values) != len(set(values)) or set(values) - pattern_ids:
                errors.append(f"{prefix} invalid or duplicate {label} pattern IDs")
        if (set(member_ids) & set(uncertain_ids)) or (set(member_ids) & set(closest_ids)) or (set(uncertain_ids) & set(closest_ids)):
            errors.append(f"{prefix} membership, uncertainty, and exclusion sets overlap")

        for item in memberships:
            if not isinstance(item, dict) or set(item) != {
                "pattern_id", "fit", "gate_evidence", "reason",
                "decisive_chain_fields", "confidence"
            }:
                errors.append(f"{prefix} malformed membership")
                continue
            if item.get("fit") not in FITS or item.get("confidence") not in CONFIDENCES:
                errors.append(f"{prefix} invalid membership fit/confidence")
            if not _text(item.get("reason"), 50):
                errors.append(f"{prefix} membership reason too short")
            if not _text(item.get("gate_evidence"), 40):
                errors.append(f"{prefix} membership gate_evidence too short")
            gate = item.get("gate_evidence", "").casefold()
            if item.get("pattern_id") == "A-P01":
                fixed_terms = ("held fixed", "holds", "holding", "controlled", "same setup")
                varied_terms = (
                    "varied", "varies", "changes", "contrast", "intervention",
                    "ablation", "replacement", "on versus off", "with versus without",
                )
                if not any(term in gate for term in fixed_terms) or not any(
                    term in gate for term in varied_terms
                ):
                    errors.append(
                        f"{prefix} A-P01 gate must state both fixed and varied contrast"
                    )
            fields = item.get("decisive_chain_fields")
            if not isinstance(fields, list) or not fields or set(fields) - CHAIN_FIELDS:
                errors.append(f"{prefix} invalid decisive_chain_fields")
            normalized = re.sub(r"\s+", " ", item.get("reason", "")).strip().casefold()
            reason_owners[normalized].add(episode_id)
        for item in uncertain:
            if not isinstance(item, dict) or set(item) != {"pattern_id", "reason", "missing_links"}:
                errors.append(f"{prefix} malformed uncertain pattern")
                continue
            if not _text(item.get("reason"), 40):
                errors.append(f"{prefix} uncertainty reason too short")
            missing = item.get("missing_links")
            if not isinstance(missing, list) or not missing or not all(_text(value, 5) for value in missing):
                errors.append(f"{prefix} uncertainty needs concrete missing_links")
        for item in closest:
            if not isinstance(item, dict) or set(item) != {"pattern_id", "reason"}:
                errors.append(f"{prefix} malformed closest excluded pattern")
                continue
            if not _text(item.get("reason"), 40):
                errors.append(f"{prefix} closest-excluded reason too short")

        unmapped = row.get("unmapped_logic")
        if unmapped is not None:
            unmapped_count += 1
            if not isinstance(unmapped, dict) or set(unmapped) != {"label", "abstract_signature", "reason"}:
                errors.append(f"{prefix} malformed unmapped_logic")
            elif not all(_text(unmapped.get(key), minimum) for key, minimum in (("label", 8), ("abstract_signature", 25), ("reason", 50))):
                errors.append(f"{prefix} unmapped_logic text too short")
        if not memberships and not uncertain and unmapped is None:
            errors.append(f"{prefix} no membership/uncertainty requires unmapped_logic")
        if row.get("classification_confidence") not in CONFIDENCES:
            errors.append(f"{prefix} invalid classification_confidence")
        if uncertain and row.get("classification_confidence") == "high":
            errors.append(f"{prefix} uncertainty is incompatible with high confidence")
        if uncertain and row.get("needs_source_audit") is not True:
            errors.append(f"{prefix} uncertain membership requires source audit")
        if not isinstance(row.get("needs_source_audit"), bool):
            errors.append(f"{prefix} needs_source_audit must be boolean")
        source_episode = source_by_id.get(episode_id, {})
        has_primary = any(
            value.get("provenance_level") == "primary"
            for value in source_episode.get("evidence", {}).values()
            if isinstance(value, dict)
        )
        missing_links = source_episode.get("quality", {}).get("missing_links", [])
        if (not has_primary or missing_links) and not row.get("needs_source_audit"):
            errors.append(
                f"{prefix} missing/provenance-limited source requires needs_source_audit"
            )
        if missing_links and row.get("classification_confidence") == "high":
            errors.append(
                f"{prefix} missing source links are incompatible with high confidence"
            )
        member_count += bool(memberships)
        uncertain_count += bool(uncertain)
        audit_count += row.get("needs_source_audit") is True

    for reason, owners in reason_owners.items():
        if reason and len(owners) >= 3:
            errors.append(
                f"exact membership reason reused across {len(owners)} episodes: {reason!r}"
            )
    return {
        "shard": shard,
        "episode_count": len(rows),
        "episodes_with_membership": member_count,
        "episodes_with_uncertainty": uncertain_count,
        "episodes_with_unmapped_logic": unmapped_count,
        "episodes_needing_source_audit": audit_count,
        "error_count": len(errors),
        "errors": errors,
    }


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--directory", type=Path, default=DEFAULT_DIR)
    parser.add_argument("--shard", type=int, action="append")
    parser.add_argument("--report", type=Path)
    args = parser.parse_args()
    manifest = json.loads((args.directory / "manifest.json").read_text(encoding="utf-8"))
    shards = args.shard or [row["shard"] for row in manifest["shards"]]
    results = [validate_shard(args.directory, shard) for shard in shards]
    output = {
        "shards": results,
        "shard_count": len(results),
        "episode_count": sum(row.get("episode_count", 0) for row in results),
        "error_count": sum(row["error_count"] for row in results),
    }
    rendered = json.dumps(output, ensure_ascii=False, indent=2) + "\n"
    if args.report:
        args.report.write_text(rendered, encoding="utf-8")
    print(rendered, end="")
    raise SystemExit(1 if output["error_count"] else 0)


if __name__ == "__main__":
    main()
