...
> **追記 (2026-08-27): 本書は歴史的記録。** データ層は下記の状態で確定し、§5 の「方針確認」は「成功分のみを統合する（残存失敗952件はリトライしない）」で決着した — 未読率は限界事項として `method.html` と HF データセットカードに明記済み。以降の作業はすべて分析・可視化・サイト層で、`method.html` §10–§11 と本リポジトリのコミット履歴が引き継ぎを兼ねる。本書の運用手順を新規セッションが実行する必要はない。
